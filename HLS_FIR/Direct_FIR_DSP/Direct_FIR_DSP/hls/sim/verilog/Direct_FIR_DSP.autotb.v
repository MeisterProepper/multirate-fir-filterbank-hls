...
`define AUTOTB_DUT      Direct_FIR_DSP
`define AUTOTB_DUT_INST AESL_inst_Direct_FIR_DSP
`define AUTOTB_TOP      apatb_Direct_FIR_DSP_top
`define AUTOTB_LAT_RESULT_FILE "Direct_FIR_DSP.result.lat.rb"
`define AUTOTB_PER_RESULT_TRANS_FILE "Direct_FIR_DSP.performance.result.transaction.xml"
`define AUTOTB_TOP_INST AESL_inst_apatb_Direct_FIR_DSP_top
`define AUTOTB_MAX_ALLOW_LATENCY  15000000
`define AUTOTB_CLOCK_PERIOD_DIV2 5.00
`define AUTOTB_II 795
`define AUTOTB_LATENCY 794

`define AESL_DEPTH_input_r 1
`define AESL_DEPTH_output_r 1
`define AUTOTB_TVIN_input_r  "../tv/cdatafile/c.Direct_FIR_DSP.autotvin_input_r.dat"
`define AUTOTB_TVIN_input_r_out_wrapc  "../tv/rtldatafile/rtl.Direct_FIR_DSP.autotvin_input_r.dat"
`define AUTOTB_TVOUT_output_r  "../tv/cdatafile/c.Direct_FIR_DSP.autotvout_output_r.dat"
`define AUTOTB_TVOUT_output_r_out_wrapc  "../tv/rtldatafile/rtl.Direct_FIR_DSP.autotvout_output_r.dat"
module `AUTOTB_TOP;

parameter AUTOTB_TRANSACTION_NUM = 460;
parameter PROGRESS_TIMEOUT = 10000000;
parameter LATENCY_ESTIMATION = 794;
...
reg [31:0] ap_c_n_tvout_trans_num_output_r;
integer done_cnt = 0;
...
AESL_axi_s_input_r AESL_AXI_S_input_r(
    ...
    .TRAN_input_r_TVALID(input_r_TVALID),
    .TRAN_input_r_TREADY(input_r_TREADY),
    .ready(input_r_ready),
    .done(input_r_done),
...
AESL_axi_s_output_r AESL_AXI_S_output_r(
    ...
    .TRAN_output_r_TVALID(output_r_TVALID),
    .TRAN_output_r_TREADY(output_r_TREADY),
...
initial begin : initial_process_for_dut_rst
    ...
    always @ (*) begin
        if (end_input_r) begin
            ready_input_r <= 1;
        end else if (ap_c_n_tvin_trans_num_input_r > ready_cnt) begin
            ready_input_r <= 1;
        end else begin
            ready_input_r <= 0;
        end
...
begin
    ...
    always @ (*) begin
        if (end_output_r) begin
            done_output_r <= 1;
        end else if (ap_c_n_tvout_trans_num_output_r > done_cnt + 1) begin
            done_output_r <= 1;
        end else if (size_output_r > 1) begin
            done_output_r <= 0;
        end else if (output_r_TVALID == 1) begin
            done_output_r <= 1;
        end else begin
            done_output_r <= 0;
        end
    ...
    `define STREAM_SIZE_OUT_output_r "../tv/stream_size/stream_size_out_output_r.dat"
    
    initial begin : proc_ap_c_n_tvout_trans_num_output_r
        integer fp_output_r;
        reg [127:0] token_output_r;
        integer ret;
        
        ap_c_n_tvout_trans_num_output_r = 0;
        end_output_r = 0;
        wait (AESL_reset === 1);
        
        fp_output_r = $fopen(`STREAM_SIZE_OUT_output_r, "r");
        if (fp_output_r == 0) begin // Failed to open file
            $display("Failed to open size file for \"%s\"!", `STREAM_SIZE_OUT_output_r);
            $finish;
        end
        read_token(fp_output_r, token_output_r);
        if (token_output_r != "[[[runtime]]]") begin
            $display("ERROR: token_output_r != \"[[[runtime]]]\"");
            $finish;
        end
        
        size_output_r = 0;
        while (size_output_r == 0 && end_output_r == 0) begin
            ap_c_n_tvout_trans_num_output_r = ap_c_n_tvout_trans_num_output_r + 1;
            read_token(fp_output_r, token_output_r); // should be [[transaction]] or [[[/runtime]]]
            if (token_output_r == "[[transaction]]") begin
                read_token(fp_output_r, token_output_r); // should be transaction number
                read_token(fp_output_r, token_output_r); // should be size for hls::stream
                ret = $sscanf(token_output_r, "%d", size_output_r);
                read_token(fp_output_r, token_output_r); // should be [[/transaction]]
            end else if (token_output_r == "[[[/runtime]]]") begin
                $fclose(fp_output_r);
                end_output_r = 1;
            end else begin
                $display("ERROR: unknown token_output_r");
                $finish;
            end
        end
        forever begin
            @ (posedge AESL_clock);
            if (end_output_r == 0) begin 
                if (output_r_TVALID == 1) begin
                    if (size_output_r > 0) begin
                        size_output_r = size_output_r - 1;
                        while (size_output_r == 0 && end_output_r == 0) begin
                            ap_c_n_tvout_trans_num_output_r = ap_c_n_tvout_trans_num_output_r + 1;
                            read_token(fp_output_r, token_output_r); // should be [[transaction]] or [[[/runtime]]]
                            if (token_output_r == "[[transaction]]") begin
